...
uint32_t targetting(dungeon *d)
{
  ...
  io_display(d);
  mvprintw(0, 0, "Select a target.  Enter to select; ESC to cancel.");
  ...
  do {
    do {
      ...
      FD_SET(STDIN_FILENO, &readfs);
      tv.tv_sec = 0;
      tv.tv_usec = 125000; /* An eighth of a second */
      io_redisplay_visible_monsters(d, dest);
    } while (!select(STDIN_FILENO + 1, &readfs, NULL, NULL, &tv));
    ...
    c = getch();
    pair_t tmp = { dest[dim_x], dest[dim_y] };
    switch (c) {
    case '7':
    case 'y':
    case KEY_HOME:
      tmp[dim_y]--;
      tmp[dim_x]--;
      if (tmp[dim_y] >= 1 && tmp[dim_x] >= 1 &&
          abs(tmp[dim_y] - d->PC->position[dim_y]) <= PC_VISUAL_RANGE &&
          abs(tmp[dim_x] - d->PC->position[dim_x]) <= PC_VISUAL_RANGE &&
          can_see(d, d->PC->position, tmp, 1, 0)) {
        dest[dim_y] = tmp[dim_y];
        dest[dim_x] = tmp[dim_x];
      }
      break;
    case '8':
    case 'k':
    ...
    case KEY_PPAGE:
      tmp[dim_y]--;
      tmp[dim_x]++;
      if (tmp[dim_y] >= 1 && tmp[dim_x] < DUNGEON_X - 1 &&
          abs(tmp[dim_y] - d->PC->position[dim_y]) <= PC_VISUAL_RANGE &&
          abs(tmp[dim_x] - d->PC->position[dim_x]) <= PC_VISUAL_RANGE &&
          can_see(d, d->PC->position, tmp, 1, 0)) {
        dest[dim_y] = tmp[dim_y];
    ...
    case KEY_NPAGE:
      tmp[dim_y]++;
      tmp[dim_x]++;
      if (tmp[dim_y] < DUNGEON_Y - 1 && tmp[dim_x] < DUNGEON_X - 1 &&
          abs(tmp[dim_y] - d->PC->position[dim_y]) <= PC_VISUAL_RANGE &&
          abs(tmp[dim_x] - d->PC->position[dim_x]) <= PC_VISUAL_RANGE &&
          can_see(d, d->PC->position, tmp, 1, 0)) {
        dest[dim_y] = tmp[dim_y];
    ...
    case KEY_END:
      tmp[dim_y]++;
      tmp[dim_x]--;
      if (tmp[dim_y] < DUNGEON_Y - 1 && tmp[dim_x] >= 1 &&
          abs(tmp[dim_y] - d->PC->position[dim_y]) <= PC_VISUAL_RANGE &&
          abs(tmp[dim_x] - d->PC->position[dim_x]) <= PC_VISUAL_RANGE &&
          can_see(d, d->PC->position, tmp, 1, 0)) {
        dest[dim_y] = tmp[dim_y];
  ...
  if (!d->PC->targetSelection(d, dest)) {
    io_queue_message("Target selected!");
    io_display(d);
    return 0;
  }
...
void io_handle_input(dungeon *d)
{
  ...
  do {
    ...
    case 'q':
      ...
      io_queue_message("Since there are multiple messages, " "you will see \"more\" prompts.");
      io_queue_message("You can use any key to advance through messages.");
      io_queue_message("Normal gameplay will not resume until the queue " "is empty.");
      io_queue_message("Long lines will be truncated, not wrapped.");
      io_queue_message("io_queue_message() is variadic and handles " "all printf() conversion specifiers.");
      io_queue_message("Did you see %s?", "what I did there");
      io_queue_message("When the last message is displayed, there will ""be no \"more\" prompt.");
      io_queue_message("Have fun!  And happy printing!");
      fail_code = 0;
      break;
    case 'r':
      fail_code = targetting(d);
      break;
    case 'F':
      fail_code = d->PC->distanceattack(d);
      break;
    case 'P':
      fail_code = d->PC->poisonBall(d);
      break;
    default:
